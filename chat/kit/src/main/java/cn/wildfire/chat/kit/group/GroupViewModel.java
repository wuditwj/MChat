...
public class GroupViewModel extends ViewModel implements OnGroupInfoUpdateListener, OnGroupMembersUpdateListener {
     ...
     * @return
     */
    public MutableLiveData<OperateResult<Boolean>> setTransferGroup(String groupId, String uid,
                                                                    List<Integer> lines,
                                                                    NotificationMessageContent notifyMsg) {
        MutableLiveData<OperateResult<Boolean>> result = new MutableLiveData<>();
        ChatManager.Instance().transferGroup(groupId,uid, lines,
                notifyMsg, new GeneralCallback() {
                    @Override
                    public void onSuccess() {
                        result.setValue(new OperateResult<>(0));
                    }

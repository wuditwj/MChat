...
import cn.wildfire.chat.app.dialog.CommonDialog;
import cn.wildfire.chat.app.envelope.RedPackageActivity;
import cn.wildfire.chat.app.util.CustomSGLayoutManager;
...
//    private LinearLayoutManager layoutManager;
    ...
    private CustomSGLayoutManager sgLayoutManager;
    ...
    private Observer<List<UserInfo>> userInfoUpdateLiveDataObserver = new Observer<List<UserInfo>>() {
        ...
        public void onChanged(@Nullable List<UserInfo> userInfos) {
            ...
            int start = sgLayoutManager.findFirstVisibleItemPosition();
            int end = sgLayoutManager.findLastVisibleItemPosition();
    ...
    private void initView() {
        ...
        sgLayoutManager = new CustomSGLayoutManager(getActivity());
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (!recyclerView.canScrollVertically(1)) {
                    ...
                    if (initialFocusedMessageId != -1 && !loadingNewMessage && shouldContinueLoadNewMessage) {
                        int lastVisibleItem = sgLayoutManager.findLastCompletelyVisibleItemPosition();
...
//            }
    ...
    private void setupConversation(Conversation conversation) {
        if (conversation.type == Conversation.ConversationType.Group) {
            ...
            boolean flag = Hawk.get(groupInfo.target, false);
            if (!flag) {
                userViewModel.setUserSetting(UserSettingScope.GroupHideNickname, groupInfo.target, "1");//默认显示
    ...
    private void updateGroupMuteStatus() {
        if (groupInfo == null || groupMember == null) {
            inputPanel.disableInput("群已解散");
            return;
        }
        if (groupInfo.mute == 1) {
            if (groupMember.type != GroupMember.GroupMemberType.Owner && groupMember.type != GroupMember.GroupMemberType.Manager) {
                inputPanel.disableInput("全员禁言中");//管理员或群主不禁言 普通成员禁言
            }
        } else if (groupInfo.mute == 2) {
            inputPanel.disableInput("本群由于严重违规，已被禁用，点击退出群聊");
            inputPanel.setOnClickListener(v -> {
                if (groupInfo != null && userViewModel.getUserId().equals(groupInfo.owner)) {
                    groupViewModel.dismissGroup(groupInfo.target, Collections.singletonList(0), null).observe(this, aBoolean -> {
                        if (aBoolean != null && aBoolean) {
                            Intent intent = new Intent(getContext(), MainActivity.class);
                            startActivity(intent);
                        } else {
                            ToastUtils.show("退出群组失败");
                        }
                    });
                } else {
                    groupViewModel.quitGroup(groupInfo.target, Collections.singletonList(0), null).observe(this, aBoolean -> {
                        if (aBoolean != null && aBoolean) {
                            Intent intent = new Intent(getContext(), MainActivity.class);
                            startActivity(intent);
                        } else {
...
//        }
    ...
    public void onPortraitLongClick(UserInfo userInfo) {
        if (conversation.type == Conversation.ConversationType.Group) {
            ...
            commonDialog.setSingle(true).setOnClickBottomListener(new CommonDialog.OnClickBottomListener() {
                @Override

    ...
    protected void onContactPicked(List<UIUserInfo> newlyCheckedUserInfos) {
        ...
        if (userInfos.size() == 1) {
            ...
            groupViewModel.createGroup(this, userInfos, null, Arrays.asList(0)).observe(this, result -> {
                dialog.dismiss();
                if (result.isSuccess()) {
                    groupViewModel.setGroupJoinType(result.getResult(), 2, null, Collections.singletonList(0))
                    ...
                    //
                    ChatManager.Instance().setUserSetting(UserSettingScope.GroupHideNickname, result.getResult(), "1", new GeneralCallback() {
                        @Override
